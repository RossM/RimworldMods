...
foreach($name in $namesToRender){
 foreach($size in @(256,128,32,24)){
  ...
  if($name -eq 'Current'){
   ...
   $cb=[System.Drawing.Rectangle]::new(8,8,112,112);$sb=$boxes[$name];$f=$size/128.0
...
$g.DrawString('CHYRR / PROPORTION STUDIES',$title,$white,24,20)
$g.DrawString('Current icon and new proportions at 24 and 32 px. View at 100%.',$small,$muted,24,59)
for($i=0;$i -lt $namesToRender.Count;$i++){
 $name=$namesToRender[$i];$x=24+$i*240
 $g.FillRectangle($panel,$x,90,216,480)
 $caption=if($name -eq 'Current'){'Current in game'}elseif($name -eq 'C'){'Revised wings'}elseif($name -eq 'Ears'){'Bat ears'}else{"Candidate $name"}
