...
public class Main(ModContentPack content) : Mod(content)
{
    ...
    private static void Check(Type type)
    {
        ...
        var fields = type.GetFields().Where(field => field.GetCustomAttribute<UnsavedAttribute>() == null &&
        ...
        {
            Log.Warning(
                $"Possibly unsaved field: {type.FullName}.{field.Name}. Either save this field in {exposeDataFunc}, mark it [Unsaved], or make it const or readonly.");
        }
    }
}
