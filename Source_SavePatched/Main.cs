...
public class Main(ModContentPack content) : Mod(content)
{
            ...
            else
            {
                switch (oldInstruction.Value)
                {
                    ...
                    case MethodInfo value:
                    {
                        ...
                        //        null);
                        //}
                        ilGenerator.Emit(oldInstruction.OpCode, value);
                        break;
                    }
    ...
    {
        return new ExceptionHandler(e.TryOffset, e.TryLength, e.FilterOffset, e.HandlerOffset, e.HandlerLength, e.Flags,
            e.CatchType?.MetadataToken ?? 0);
    }
}
